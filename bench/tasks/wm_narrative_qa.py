...
TASK_NAME = "wm_narrative_qa"
TASK_NAME_SUM = "sum_narrative_qa"

RECALL_PREAMBLE = wm_mcq_recall_preamble(
    task_prompt=TASK_DESC,
    answer_focus="about the story you read",
)
...
def evaluate(
    ...
    *,
    ...
    data_json_path: str = "data/narrative_QA.json",
    ...
    if n_stories is not None:
        pool = rng.sample(pool, min(int(n_stories), len(pool)))
    ...
    for cond_id in CONDITIONS:
        ...
        def _one_participant(pid: int, cond_id: str = cond_id) -> Dict[str, Any]:
            ...
            if debug:
                print(f"  {TASK_NAME} | {cond_id} | p{pid} | story={story_id}")
                print(f"  answers: {answer_map}")
                print(f"  accuracy: {scored['accuracy']:.2f}")
                print()

            row = {
                "id": f"{TASK_NAME}:{cond_id}:p{pid}:{story_id}",
                "condition_id": cond_id,
        ...
        cond_summary = summarize_story_rows(rows, cond_id, CONDITIONS[cond_id]["name"])
    ...
    *,
    ...
    for cond_id in SUMMARIZER_CONDITIONS:
        ...
        def _one_participant(pid: int, cond_id: str = cond_id) -> Dict[str, Any]:
            ...
            if debug:
                print(f"  {TASK_NAME_SUM} | {cond_id} | p{pid} | story={story_id}")
                print(f"  answers: {answer_map}")
                print(f"  accuracy: {scored['accuracy']:.2f}")
                print()

            row = {
                "id": f"{TASK_NAME_SUM}:{cond_id}:p{pid}:{story_id}",
                "condition_id": cond_id,
        ...
        cond_summary = summarize_story_rows(

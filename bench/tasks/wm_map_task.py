...
TASK_NAME = "wm_map_task"
TASK_NAME_SUM = "sum_map_task"

RECALL_PREAMBLE = wm_mcq_recall_preamble(
    task_prompt=TASK_DESC,
    answer_focus="about routes on the map",
)
...
def evaluate(
    ...
    *,
    ...
    data_json_path: str = "data/maps.json",
    ...
    for cond_id in CONDITIONS:
        ...
        def _one_participant(pid: int, cond_id: str = cond_id) -> List[Dict[str, Any]]:
            ...
            for item in items:
                ...
                encode_content = map_to_text(item)
                ...
                if debug:
                    print(f"  {TASK_NAME} | {cond_id} | p{pid} | map={map_id}")
                    print(f"  answers: {answer_map}")
                    print(f"  accuracy: {scored['accuracy']:.2f}")
                    print()

                row = {
                    "id": f"{TASK_NAME}:{cond_id}:p{pid}:{map_id}",
                    "condition_id": cond_id,
        ...
        cond_summary = summarize_map_rows(rows, cond_id, CONDITIONS[cond_id]["name"])
    ...
    for cond_id in SUMMARIZER_CONDITIONS:
        ...
        def _one_participant(pid: int, cond_id: str = cond_id) -> List[Dict[str, Any]]:
            ...
            for item in items:
                ...
                if debug:
                    print(f"  {TASK_NAME_SUM} | {cond_id} | p{pid} | map={map_id}")
                    print(f"  answers: {answer_map}")
                    print(f"  accuracy: {scored['accuracy']:.2f}")
                    print()

                row = {
                    "id": f"{TASK_NAME_SUM}:{cond_id}:p{pid}:{map_id}",
                    "condition_id": cond_id,
        ...
        rows = [row for p_rows in per_participant for row in p_rows]
        cond_summary = summarize_map_rows(

...
TASK_NAME = "wm_craft_task"
TASK_NAME_SUM = "sum_craft_task"

RECALL_PREAMBLE = wm_mcq_recall_preamble(
    task_prompt=TASK_DESC,
    answer_focus="about how items combine",
)
...
def _format_questions(questions: List[Dict[str, Any]]) -> str:
    ...
    for i, q in enumerate(questions, start=1):
        ...
        lines.append(f"Question {i}: {q.get('prompt', '')} {opts}")
    return "\n".join(lines)
...
def evaluate(
    ...
    *,
    ...
    data_json_path: str = "data/craft_task.json",
    ...
    for cond_id in CONDITIONS:
        ...
        def _one_participant(pid: int, cond_id: str = cond_id) -> List[Dict[str, Any]]:
            participant_rows = []
            for item in items:
                task_id = item.get("task_id", "")
                questions = item.get("questions", [])
                encode_content = craft_to_text(item)
                ...
                if debug:
                    print(f"  {TASK_NAME} | {cond_id} | p{pid} | task={task_id}")
                    print(f"  answers: {answer_map}")
                    print(f"  accuracy: {scored['accuracy']:.2f}")
                    print()

                row = {
                    "id": f"{TASK_NAME}:{cond_id}:p{pid}:{task_id}",
                    "condition_id": cond_id,
        ...
        per_participant = map_participants(
            list(range(1, n_participants + 1)),
            _one_participant,
            max_workers=workers,
        )
        rows = [row for p_rows in per_participant for row in p_rows]
        cond_summary = summarize_craft_rows(rows, cond_id, CONDITIONS[cond_id]["name"])
    ...
    for cond_id in SUMMARIZER_CONDITIONS:
        ...
        def _one_participant(pid: int, cond_id: str = cond_id) -> List[Dict[str, Any]]:
            ...
            for item in items:
                ...
                if debug:
                    print(f"  {TASK_NAME_SUM} | {cond_id} | p{pid} | task={task_id}")
                    print(f"  answers: {answer_map}")
                    print(f"  accuracy: {scored['accuracy']:.2f}")
                    print()

                row = {
                    "id": f"{TASK_NAME_SUM}:{cond_id}:p{pid}:{task_id}",
                    "condition_id": cond_id,
        ...
        rows = [row for p_rows in per_participant for row in p_rows]
        cond_summary = summarize_craft_rows(

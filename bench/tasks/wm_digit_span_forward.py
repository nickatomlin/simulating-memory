...
TASK_NAME = "wm_digit_span_forward"
TASK_NAME_SUM = "sum_digit_span_forward"
...
CANDIDATE_PROMPT = """\
You are segmenting a digit sequence into memorable chunks for a human memory experiment.

Sequence with positions:
{indexed_sequence}

Propose up to {max_keys} NON-OVERLAPPING, contiguous segments that together cover as much \
of the sequence as possible, reading left to right.

For each segment, output exactly one line:
  pos <start>-<end>: <digits> | <mnemonic (3 words max)>

A multi-digit chunk is only worth keeping if the pattern is IMMEDIATELY OBVIOUS:
  GOOD: "7 7 7 | triple sevens"
        "1 2 3 4 | counting up"
        "1 9 9 9 | year 1999"
        "9 8 7 | countdown"
        "0 0 | double zero"
  BAD:  "3 8 1 | descending then ascending"   (not obvious, too wordy)
        "6 2 4 | skip-two pattern"             (requires explanation)
        "4 1 9 | year 419"                     (obscure year, not salient)

IMPORTANT: A single digit is also a valid chunk.
Output ONLY the candidate list, one line per candidate. No extra text."""
...
FORWARD_RECALL_PROMPT = wm_recall_prompt(
    task_prompt=TASK_DESC,
    wm_recall_instructions=(
        "reconstruct the digit sequence by reading digits from the first key, then the second "
        "key, and so on in order. Output the digits from your memory chunks, then press <<S>>. "
...
Digit sequence (with positions):
{indexed_sequence}

Write a short summary describing this sequence — mention salient chunks, \
patterns, or landmarks (e.g. "triple sevens at pos 3-5", "counts up 1-2-3-4 \
then 9"). Your summary should be brief but detailed enough that you can later \
reconstruct the full sequence in order from it."""

SUM_FORWARD_RECALL_PROMPT = summarizer_recall_prompt(
    task_prompt=TASK_DESC,
    recall_instructions=(
        "reconstruct the digit sequence by reading through your summary in order. "
        "Output the digits, then press <<S>>. No extra text, no explanations."
    ),
    format_rules=FORMAT_RULES + "\nAfter the final digit, output:\npress <<S>>.",
)
...
def _indexed_sequence(digits: List[int]) -> str:
    """Format digits with 1-based position numbers for prompt clarity.

    Example: [4, 1, 9] -> 'pos: 1  2  3\\ndig: 4  1  9'
    """
    positions = "  ".join(str(i + 1) for i in range(len(digits)))
    values = "  ".join(str(d) for d in digits)
    return f"pos: {positions}\ndig: {values}"


def encode_digit_span(
    agent: WorkingMemoryAgent,
    llm: LLM,
    digits: List[int],
    temperature: float,
) -> Tuple[str, Dict[str, Any]]:
    """Two-step encoding: candidate generation then WM storage.

    Returns
    -------
    candidates_text : str
        Raw candidate chunk ideas from step 1.
    encoding_log : dict
        Tool-call log from step 2 (write_memory calls + final KV).
    """
    indexed = _indexed_sequence(digits)

    # Step 1: generate candidate chunk ideas (position-aware, strict saliency)
    cand_resp = llm.generate(
        CANDIDATE_PROMPT.format(indexed_sequence=indexed, max_keys=MAX_KEYS),
        temperature=temperature,
        max_tokens=300,
    )
    candidates_text = cand_resp.text or ""

    # Step 2: agent selects non-overlapping chunks and stores via tools
    encode_input = f"Digit sequence:\n{indexed}\n\nCandidate chunks:\n{candidates_text}"
    encoding_log = agent.encode(encode_input)
    return candidates_text, encoding_log


def encode_digit_span_summary(
    agent: SummarizerAgent,
    digits: List[int],
) -> Dict[str, Any]:
    indexed = _indexed_sequence(digits)
    encode_input = DIGIT_SUMMARIZE_ENCODE_PROMPT.format(indexed_sequence=indexed)
    return agent.encode(encode_input)
...
def evaluate(
    ...
    *,
    ...
    # Phase 1: pre-generate the full (participant x condition x span x repeat) grid.
    ...
    # Phase 2: run every cell independently.
    def _one_trial(i: int) -> Dict[str, Any]:
        ...
        recall_raw = agent.recall(recall_prompt=FORWARD_RECALL_PROMPT)
    ...
    # Phase 3: aggregate with the original early-stop semantics.
    ...
    for participant_id in range(1, n_participants + 1):
        for cond_id in CONDITIONS:
            ...
            cond_prefix[cond_id].append(
                sum(r["metrics"]["prefix_acc"] for r in kept_rows) / n if n else 0.0
            )
...
def evaluate_summarizer(
    ...
    *,
    ...
    def _one_trial(i: int) -> Dict[str, Any]:
        ...
        encoding_log = encode_digit_span_summary(agent, digits)
        recall_raw = agent.recall(recall_prompt=SUM_FORWARD_RECALL_PROMPT)
        pred = parse_pressed_digits(recall_raw)
        has_s = "<<S>>" in recall_raw or "<<s>>" in recall_raw.lower()
